...
- Known issue: the shared Supabase dev DB's `flyway_schema_history` reports current version 17, but `library_contents.submitted_at` already exists (i.e. V18's effect is present without V18 being recorded as applied). This blocks `./mvnw spring-boot:run` against that DB until the schema history is reconciled manually — V19/V20 (Community Hub moderation/comments/reports) could not be runtime-verified against the cloud DB because of this pre-existing drift, unrelated to this session's changes.
