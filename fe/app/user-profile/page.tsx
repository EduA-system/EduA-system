...
function UserProfileContent() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!isStudent) {
      const validationError = phoneNumberError(phoneNumber) ?? dateOfBirthError(dateOfBirth);
      if (validationError) {
              ...
              <div className="rounded-[14px] border border-[#d8d1c9] bg-white px-5 py-6 sm:px-7">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <label className="text-[12px] font-medium text-[#6b6b6b] sm:col-span-2">Ngày sinh
                    <input type="date" value={dateOfBirth} onChange={(event) => setDateOfBirth(event.target.value)} min={dateYearsAgo(MAX_EDUCATOR_AGE)} max={dateYearsAgo(MIN_EDUCATOR_AGE)} required={!isStudent} disabled={isStudent || saving} className={fieldClassName(isStudent)} />
                  </label>
