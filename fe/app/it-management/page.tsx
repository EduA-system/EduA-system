...
function ItManagementContent() {
  ...
  const load = useCallback(async () => {
    ...
    setMessage("");
    try {
      const response = await authFetch("/api/it-management/system-prompts");
  ...
  async function save(prompt: EditablePrompt) {
    ...
    setMessage("");
    try {
      const response = await authFetch(`/api/it-management/system-prompts/${prompt.key}`, {
        method: "PUT",
  ...
  return (
    <main className="min-h-screen bg-[#f5f1ec] text-[#1f1f1f]">
      ...
      <div className="flex min-h-[calc(100vh-3.5rem)] md:min-h-screen">
        <Sidebar responsive mobileOpen={mobileMenuOpen} activeHref="/it-management" />
        <section className="min-w-0 flex-1 px-5 py-8 sm:px-8 lg:px-12 lg:py-10">
          <div className="mx-auto max-w-7xl">
            <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
              <div className="max-w-3xl">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-[#d97757]">IT Management</p>
            ...
            <div className="mt-8 grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)]">
              <aside className="space-y-4">
                ...
                <div className="rounded-lg border border-[#d8d1c9] bg-[#fbfaf8] p-4 text-sm text-[#6b6b6b] shadow-[0_2px_8px_rgba(43,41,38,0.04)]">
                  ...
                  <p className="mt-1 text-xs uppercase tracking-[0.12em] text-[#d97757]">Quản lý CNTT</p>
                  <button
                    type="button"
                    onClick={() => void load()}
                    disabled={loading}
...
export default function ItManagementPage() {
  return (
    <RouteGuard pathname="/it-management">
      <ItManagementContent />
    </RouteGuard>
  );
}
