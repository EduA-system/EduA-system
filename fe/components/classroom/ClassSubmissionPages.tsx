...
export function ClassSubmissionsPage() { const state = useResource(); return <ClassHubFrame classId={state.classId} active="assignments"><LoadingOrError resource={state.resource} error={state.error} />{state.resource && <div className="mt-6"><SubmissionsRosterPanel authFetch={state.authFetch} classId={state.classId} resource={state.resource} /></div>}</ClassHubFrame>; }
export function ClassSubmissionDetailPage() { const state = useResource(); const studentId = useSearchParams().get("studentId") ?? ""; return <ClassHubFrame classId={state.classId} active="assignments"><LoadingOrError resource={state.resource} error={state.error} />{state.resource && studentId && <div className="mt-6"><SubmissionDetailPanel authFetch={state.authFetch} classId={state.classId} resource={state.resource} studentId={studentId} /></div>}</ClassHubFrame>; }
