...
import { type ClassResourceSummary, formatFileSize, getClassDetail, listClassResources, sourceTypeLabel } from "@/lib/classroom";
...
export function TeacherResourceDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setClassInactive(detail.status === "INACTIVE");
  ...
  return (
    ...
    >
      <div className="mt-6">
        ...
        {loading ? <div className="mt-6 flex items-center gap-2 text-sm text-[#6b6b6b]"><Loader2 className="size-4 animate-spin" /> Đang tải tài nguyên...</div> : error ? <div className="mt-6 rounded-[14px] border border-dashed border-[#e8b4a4] bg-[#fdf3ef] px-5 py-12 text-center text-[13px] text-[#c0492b]">{error}</div> : resource ? <ResourceDetail resource={resource} classId={classId} classInactive={classInactive} /> : <div className="mt-6 rounded-[14px] border border-dashed border-[#d8d1c9] px-5 py-12 text-center text-[13px] text-[#6b6b6b]">Chọn một tài nguyên để xem chi tiết.</div>}
      </div>
    </ClassHubFrame>
  );
}

function ResourceDetail({ resource, classId, classInactive }: { resource: ClassResourceSummary; classId: string; classInactive?: boolean }) {
  ...
  return <article className="mt-5 rounded-[14px] border border-[#d8d1c9] bg-white p-5 sm:p-7">
    {!classInactive && <div className="flex flex-wrap items-center gap-2">
    ...
    <h2 className={`text-[25px] font-semibold leading-tight text-[#1f1f1f] ${classInactive ? "" : "mt-4"}`}>{resource.title}</h2>
    ...
    <div className="mt-6 flex flex-wrap items-center gap-2 border-t border-[#ede8e1] pt-4 text-[12px] text-[#817a72]">
      ...
      {!classInactive && resource.deadline && <><span aria-hidden>·</span><span className={`inline-flex items-center gap-1.5 rounded-[10px] border px-2.5 py-1.5 font-medium ${deadlineClasses(resource.deadline)}`}><CalendarClock className="size-3.5" />{isOverdue(resource.deadline) ? "Quá hạn" : "Hạn nộp"}: {formatDateTime(resource.deadline)}</span></>}
    </div>
