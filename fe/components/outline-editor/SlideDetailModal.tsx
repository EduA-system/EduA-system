...
const inputClass = "w-full rounded-lg border border-[#d8d1c9] bg-[#faf9f7] px-3 py-2 text-sm text-[#1f1f1f] shadow-[0_1px_2px_rgba(43,41,38,0.03)] outline-none transition placeholder:text-[#a8a097] hover:border-[#c9beb4] hover:bg-white focus:border-[#d97757] focus:bg-white focus:shadow-[0_0_0_3px_rgba(217,119,87,0.12)]";
// Borderless variant for the basic panel, where fields share one container and are split by thin dividers.
const bareInputClass = "w-full rounded-md bg-transparent px-2 py-1.5 text-sm text-[#1f1f1f] outline-none transition placeholder:text-[#a8a097] focus:bg-[#fff7f1]/60";
...
export function BlockFields({ block, onChange, fieldClass = inputClass }: { block: ContentBlock; onChange: (block: ContentBlock) => void; fieldClass?: string }) {
  ...
  if (block.kind === "table") return <div className="grid gap-2">
    ...
    <button type="button" className="text-left text-xs font-medium text-[#d97757] transition hover:text-[#b95738]" onClick={() => onChange({ ...block, rows: [...block.rows, { id: `${block.id}-row-${Date.now()}`, cells: Array(block.columns.length).fill("") }] })}>+ Thêm hàng</button>
  </div>;
  if (block.kind === "sequence") return <div className="grid gap-2">{block.steps.map((step, index) => <div key={step.id} className="flex gap-2"><input className={`${fieldClass} w-28`} value={step.label ?? ""} onChange={(event) => { const steps = [...block.steps]; steps[index] = { ...step, label: event.target.value }; onChange({ ...block, steps }); }} placeholder={`Bước ${index + 1}`} /><input className={fieldClass} value={step.text} onChange={(event) => { const steps = [...block.steps]; steps[index] = { ...step, text: event.target.value }; onChange({ ...block, steps }); }} placeholder="Nội dung bước" /><button type="button" onClick={() => onChange({ ...block, steps: block.steps.filter((_, stepIndex) => stepIndex !== index) })}>×</button></div>)}<button type="button" className="text-left text-xs font-medium text-[#d97757] transition hover:text-[#b95738]" onClick={() => onChange({ ...block, steps: [...block.steps, { id: `${block.id}-step-${Date.now()}`, text: "" }] })}>+ Thêm bước</button></div>;
...
function RelationshipEditor({ slide, onChange }: { slide: SlideItem; onChange: (slide: SlideItem) => void }) {
  ...
  return <section className="grid gap-2"><div className="text-xs font-semibold text-[#6b6b6b]">Quan hệ block</div>{relationships.map((relationship, index) => <div key={`${relationship.type}-${index}`} className="grid grid-cols-[120px_1fr_1fr_auto] gap-2">
  ...
  </div>)}<button type="button" className="text-left text-xs font-medium text-[#d97757] transition hover:text-[#b95738]" onClick={() => { const [a, b] = blocks.map((block) => block.id); setRelationships([...relationships, { type: "supports", supportingBlockId: a ?? "", targetBlockId: b ?? a ?? "" }]); }}>+ Thêm quan hệ</button></section>;
}
...
export function SlideBasicFields({ slide, onChange }: { slide: SlideItem; onChange: (slide: SlideItem) => void }) {
  ...
  return (
    <div className="grid gap-3">
      <div className="divide-y divide-[#ebe4dc] overflow-hidden rounded-xl border border-[#d8d1c9] bg-white">
        <div className="px-3 py-2">
          <div className="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-[#8a8178]">Tiêu đề slide</div>
        ...
        {slide.contentPlan.blocks.map((block, index) => (
          <div key={block.id} className="px-3 py-2">
            <div className="mb-0.5 text-[10px] font-semibold uppercase tracking-wide text-[#8a8178]">{blockKindLabel(block)}</div>
...
export function SlideDetailPanel({ slide, onChange }: { slide: SlideItem; onChange: (slide: SlideItem) => void }) {
  ...
  return (
    <div className="grid gap-4">
      ...
      </article>)}<button type="button" className="text-left text-sm font-medium text-[#d97757] transition hover:text-[#b95738]" onClick={() => setBlocks([...slide.contentPlan.blocks, makeBlock("text")])}>+ Thêm block</button></section>
...
export function SlideDetailModal({ slide, onChange, onClose }: { slide: SlideItem; onChange: (slide: SlideItem) => void; onClose: () => void }) {
  ...
  return createPortal(<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 py-8" onClick={onClose}><div className="flex max-h-full w-full max-w-3xl flex-col overflow-hidden rounded-[14px] border border-[#d8d1c9] bg-white shadow-xl" onClick={(event) => event.stopPropagation()}>
    <header className="flex items-center justify-between border-b border-[#e8e2d9] px-5 py-4"><div><span className="rounded bg-[#fff1e9] px-2 py-1 text-xs text-[#d97757]">{slideRoleLabel(slide)}</span><span className="ml-2 text-sm font-semibold">Nội dung có cấu trúc</span></div><button type="button" onClick={onClose}>✕</button></header>
    <div className="overflow-y-auto px-5 py-4"><SlideDetailPanel slide={slide} onChange={onChange} /></div>
    <footer className="border-t border-[#e8e2d9] px-5 py-3 text-right"><button type="button" onClick={onClose} className="rounded-xl bg-[#e8724a] px-5 py-2 text-sm font-semibold text-white transition hover:bg-[#d96a42]">Xong</button></footer>
