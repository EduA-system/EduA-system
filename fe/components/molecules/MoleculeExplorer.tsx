...
export function MoleculeExplorer() {
  ...
  async function save() {
    ...
    try {
      await createLibraryContent(authFetch, {
        ...
        payload: { molecule, mode, rotating },
      });
      setMessageTone("info");
  ...
  async function buildWithAi() {
    ...
    if (readOnlyClassResource || !input || loading) return;
    setLoading(true);
    setMessage("");
    try {
        ...
        <div className="px-3 pb-3 pt-4">
          ...
          <div className="mt-2 flex h-10 items-center rounded-[13px] border border-[#ded7ce] bg-white px-3 shadow-sm focus-within:border-[#d97757] focus-within:ring-2 focus-within:ring-[#d97757]/10">
            ...
            <input
              ...
              placeholder={readOnlyClassResource ? "Chế độ chỉ xem" : "Tên hoặc công thức"}
          ...
          <button
            ...
            disabled={readOnlyClassResource || loading || !aiInput.trim()}
          ...
          <header className="relative z-10 flex flex-col gap-2.5 border-b border-[#eee9e3] px-4 py-3 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {!readOnlyClassResource ? (
                <button type="button" onClick={() => void save()} className="inline-flex h-9 items-center gap-1.5 rounded-[10px] bg-[#d97757] px-3.5 text-[11px] font-semibold text-white transition-[transform,background-color] hover:bg-[#c86748] active:scale-[0.98]">
                  <Save className="size-3.5" /> Lưu vào thư viện
                </button>
              ) : (
