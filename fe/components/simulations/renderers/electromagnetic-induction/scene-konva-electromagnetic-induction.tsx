...
export function SceneKonvaElectromagneticInduction({scene,running,resetSignal,onRunningChange,speed=1}:{scene:ElectromagneticInductionScene;running:boolean;resetSignal:number;onRunningChange:(r:boolean)=>void;speed?:number}){
 ...
 useEffect(()=>{const container=ref.current,{width:W,height:H}=size;if(!container||!W||!H)return;
  ...
  layer.add(new Konva.Text({x:18,y:16,text:"Kéo nam châm xuyên qua cuộn dây và quan sát chiều lệch của kim",fontSize:15,fill:"#e2e8f0"}));
  ...
  coil.add(new Konva.Rect({x:coilX-48,y:coilY-46,width:96,height:92,cornerRadius:8,fill:"#92400e",stroke:"#fbbf24",strokeWidth:2}));
  for(let i=0;i<13;i++){coil.add(new Konva.Ellipse({x:coilX-38+i*6.3,y:coilY,radiusX:12,radiusY:38,stroke:"#fbbf24",strokeWidth:2.2,opacity:.95}));}
  coil.add(new Konva.Text({x:coilX-48,y:coilY+57,width:96,text:"CUỘN DÂY",align:"center",fontSize:12,fontStyle:"bold",fill:"#fde68a"}));
  ...
  layer.add(new Konva.Rect({x:meterCenter.x-meterR-12,y:meterCenter.y-10,width:(meterR+12)*2,height:62,cornerRadius:8,fill:"#111827",stroke:"#64748b",strokeWidth:2}));
  layer.add(new Konva.Text({x:meterCenter.x-50,y:meterCenter.y+28,width:100,text:"ĐIỆN KẾ",align:"center",fontSize:12,fontStyle:"bold",fill:"#e2e8f0"}));
  for(let i=0;i<=10;i++){const a=Math.PI+(Math.PI*i)/10,x1=meterCenter.x+Math.cos(a)*(meterR-15),y1=meterCenter.y+Math.sin(a)*(meterR-15),x2=meterCenter.x+Math.cos(a)*(meterR-7),y2=meterCenter.y+Math.sin(a)*(meterR-7);layer.add(new Konva.Line({points:[x1,y1,x2,y2],stroke:i===5?"#ef4444":"#94a3b8",strokeWidth:i===5?2:1}));}
  layer.add(new Konva.Text({x:meterCenter.x-9,y:meterCenter.y-meterR+15,text:"0",fontSize:12,fontStyle:"bold",fill:"#dc2626"}));
  const needle=new Konva.Line({x:meterCenter.x,y:meterCenter.y,points:[0,0,0,-meterR+18],stroke:"#dc2626",strokeWidth:3,lineCap:"round"});layer.add(needle,new Konva.Circle({x:meterCenter.x,y:meterCenter.y,radius:6,fill:"#e2e8f0"}));
  ...
  const status=new Konva.Text({x:0,y:H-42,width:W,align:"center",fontSize:14,fontStyle:"bold",fill:"#e2e8f0"});layer.add(status);
 ...
 return <div ref={ref} className="h-full w-full overflow-hidden rounded-lg bg-[#0f172a]"/>;
}
