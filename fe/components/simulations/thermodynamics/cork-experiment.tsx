...
const PARAM_SCHEMA: ParamDef[] = [
  ...
  { key: "initialTemperature", label: "Nhiệt độ ban đầu", unit: "K", min: 285, max: 320, step: 1 },
  { key: "atmospherePressure", label: "Áp suất khí quyển", unit: "kPa", min: 85, max: 110, step: 0.5 },
  { key: "heatLoss", label: "Mức thất thoát nhiệt", unit: "W/K", min: 0, max: 0.6, step: 0.02 },
];
...
function MetricGrid({ live }: { live: ThermalMetrics }) {
  const items = [
    ["Nhiệt độ", `${live.temperature.toFixed(1)} K`], ["Áp suất", `${(live.pressure / 1000).toFixed(1)} kPa`],
  ...
  return <div className="grid grid-cols-2 gap-2">{items.map(([label, value]) => <div key={label} className="rounded-[8px] bg-[#faf9f7] px-3 py-2"><div className="text-[10px] uppercase tracking-wide text-[#8a8178]">{label}</div><div className="mt-0.5 font-mono text-[13px] font-semibold text-[#171717]">{value}</div></div>)}</div>;
}
...
export function CorkExperiment({ onBack }: { onBack: () => void }) {
  ...
  const panelValues = useMemo(() => ({ ...params, corkMass: params.corkMass * 1000 }), [params]);
  const runAi = () => { setAiState("thinking"); window.setTimeout(() => setAiState("review"), 1400); };

  useEffect(() => {
    if (live.time - lastPoint.current >= 0.2) { lastPoint.current = live.time; setPoints((old) => [...old, { time: live.time, temperature: live.temperature, pressure: live.pressure / 1000, heat: live.heatIn, deltaU: live.deltaInternalEnergy, work: live.work }].slice(-180)); }
