...
import { buildKernel, readPosition, readVelocity, stepScene } from "./build-derivs";
...
describe("curveTrack constraint", () => {
  ...
  it("returns to the symmetric height on a frictionless U track when preserving mechanical energy", () => {
    const points = Array.from({ length: 201 }, (_, index) => {
      const t = index / 200;
      return { x: -3 + 6 * t, y: 0.25 + 2 * (1 + Math.sin(Math.PI + Math.PI * t)) };
    });
    const scene: Scene = {
      bodies: [{ id: "ball", x: -2.7, y: points[10]!.y, vx: 0, vy: 0, mass: 1 }],
      forces: [{ kind: "gravity", g: 9.8 }],
      constraints: [{ kind: "curveTrack", body: "ball", points, friction: 0, preserveMechanicalEnergy: true }],
    };
    const kernel = buildKernel(scene);
    let state = kernel.project(kernel.initialState);
    let maxX = readPosition(state, "ball").x;
    for (let index = 0; index < 4 * 240; index += 1) {
      state = stepScene(kernel, state, 1 / 240);
      maxX = Math.max(maxX, readPosition(state, "ball").x);
    }
    expect(maxX).toBeCloseTo(2.7, 4);
  });
});
