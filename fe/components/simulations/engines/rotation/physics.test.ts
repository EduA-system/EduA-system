...
describe("rotation engine", () => {
  ...
  it("giữ moment không đổi khi dây quấn kéo tiếp tuyến trên đĩa", () => {
    const scene: RotationScene = {
      ...baseScene,
      torqueModel: "fixedTangential",
      left: { ...baseScene.left, mass: 3 },
    };
    expect(rotationTorques(scene, Math.PI / 3).net).toBeCloseTo(
      rotationTorques(scene, 0).net,
    );
  });

  it("cập nhật moment theo đường dây khi điểm móc quay cùng đĩa", () => {
    const scene: RotationScene = {
      ...baseScene,
    ...
    expect(rotationTorques(scene, 0).net).toBeCloseTo(0);
    expect(rotationTorques(scene, Math.PI / 8).net).not.toBeCloseTo(0);
  });

  it("dao động tắt dần về vị trí moment cân bằng mà không chạm chốt góc", () => {
    const scene: RotationScene = {
      ...baseScene,
      diskRadius: 1.8,
      diskMass: 6,
      ...
      left: { ...baseScene.left, mass: 0.2, radius: 1.4 },
      right: { ...baseScene.right, mass: 0.6, radius: 1.2 },
      minTheta: undefined,
      maxTheta: undefined,
    };
    const moving = rotationStateAt(scene, 2);
    const settled = rotationStateAt(scene, 24);
    expect(moving.theta).toBeLessThan(0);
    expect(settled.stoppedAtLimit).toBe(false);
    expect(Math.abs(settled.omega)).toBeLessThan(0.02);
    expect(Math.abs(rotationTorques(scene, settled.theta).net)).toBeLessThan(0.03);
  });
