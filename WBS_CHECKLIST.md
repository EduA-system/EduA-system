...
| [x] | Click-to-simulate during Presentation | Slide | Presentation | Iteration 2 | Coded | Slide simulations support `molecule`, `periodic-element`, `periodic-table`, and `sandbox` (physics experiment) elements. Molecule renders a preview in the editor and activates its live `MoleculeViewer` only after the presenter clicks “Nhấn để mô phỏng”; periodic elements/tables render from local periodic-table data in editor/presentation (`Canvas`, `ElementView`, `SlidePresentationClient`). Sandbox elements never auto-run — they show a static poster and boot Sandpack only on an explicit click (presentation) or the “Chạy thử” button (editor), since each one spawns a bundler iframe. An AI-proposed physics experiment owns its whole slide (layout topology `physics-stage`): title on top, simulation bleeding to the canvas edges, accompanying text dropped. |
...
| [~] | View Periodic Table | Simulation | Periodic Table | Iteration 2 | Partially Coded | UC-57 - Partially coded: 118-element table, filters, detail cards and 3D atom; slide editor/generation can embed compact periodic element/table simulations from local data, while lesson embedding is not implemented |
| [~] | Physics Hub (view, detail, customize via AI) | Simulation | Physics Simulation | Iteration 2 | Partially Coded | UC-53, UC-54, UC-55 - Coded frontend: /mo-phong-vat-ly provides reviewed presets and interactive physics simulations; physics experiments can now also be embedded in slides as `sandbox` elements, inserted manually from the slide editor or proposed by AI via the `physics` outline block for Vật lý lessons (`designs/slide-sandbox-physics-blocks-plan.md`). Missing: backend AI customization and persistence of customized simulations in Personal Library. |
