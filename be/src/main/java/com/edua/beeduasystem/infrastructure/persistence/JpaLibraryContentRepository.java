...
public class JpaLibraryContentRepository implements LibraryContentRepository {
    ...
    @Override @Transactional public LibraryContent save(LibraryContent c) {
        ...
        e.setStatus(c.status()); e.setPayload(c.payload()); e.setThumbnailUrl(c.thumbnailUrl()); e.setCreatedAt(c.createdAt()); e.setUpdatedAt(c.updatedAt()); e.setDeletedAt(c.deletedAt());
    ...
    @Override @Transactional(readOnly = true) public SearchResult search(UUID ownerId, LibraryContentType type, Subject subject, Integer grade, String q, int page, int size, boolean titleAscending) {
        Specification<LibraryContentEntity> spec = (root, cq, cb) -> { List<Predicate> ps = new ArrayList<>(); ps.add(cb.equal(root.get("ownerId"), ownerId)); ps.add(cb.isNull(root.get("deletedAt")));
            if (type != null) ps.add(cb.equal(root.get("type"), type)); if (subject != null) ps.add(cb.equal(root.get("subject"), subject)); if (grade != null) ps.add(cb.equal(root.get("grade"), grade));
    ...
    private static LibraryContent toDomain(LibraryContentEntity e) { return new LibraryContent(e.getId(),e.getOwnerId(),e.getType(),e.getTitle(),e.getSubject(),e.getGrade(),e.getStatus(),e.getPayload(),e.getThumbnailUrl(),e.getCreatedAt(),e.getUpdatedAt(),e.getDeletedAt()); }
}
