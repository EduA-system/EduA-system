...
public class LibraryContentService {
    ...
    public LibraryViews.Page list(String rawType, String rawSubject, Integer grade, String q, int page, int size, String sort) {
        return toPage(repository.search(currentUser.requireUserId(), parseType(rawType), parseSubject(rawSubject), parseGrade(grade), q, page, size, "title".equalsIgnoreCase(sort)), page, size);
    }
    ...
    public LibraryViews.Detail create(String rawType, String title, String rawSubject, Integer grade, JsonNode payload, String thumbnailUrl) {
        ...
        return toDetail(repository.save(new LibraryContent(UUID.randomUUID(), currentUser.requireUserId(), type, requiredTitle(title), parseSubject(rawSubject), parseGrade(grade), LibraryContentStatus.PRIVATE, payload == null ? JsonNodeFactory.instance.objectNode() : payload, cleanUrl(thumbnailUrl), now, now, null)));
    }
    public LibraryViews.Detail update(UUID id, String title, String rawSubject, boolean subjectProvided, Integer grade, boolean gradeProvided, JsonNode payload, boolean payloadProvided, String thumbnailUrl, boolean thumbnailProvided) {
        ...
        return toDetail(repository.save(new LibraryContent(c.id(),c.ownerId(),c.type(), title == null ? c.title() : requiredTitle(title), subjectProvided ? parseSubject(rawSubject) : c.subject(), gradeProvided ? parseGrade(grade) : c.grade(), c.status(), payloadProvided ? (payload == null ? JsonNodeFactory.instance.objectNode() : payload) : c.payload(), thumbnailProvided ? cleanUrl(thumbnailUrl) : c.thumbnailUrl(), c.createdAt(), Instant.now(), null)));
    }
    public void delete(UUID id) { LibraryContent c = requireOwner(id); repository.save(new LibraryContent(c.id(),c.ownerId(),c.type(),c.title(),c.subject(),c.grade(),c.status(),c.payload(),c.thumbnailUrl(),c.createdAt(),Instant.now(),Instant.now())); }
    ...
    private static LibraryViews.Summary toSummary(LibraryContent c) { return new LibraryViews.Summary(c.id(),c.type(),c.title(),c.subject(),c.grade(),c.status(),c.thumbnailUrl(),c.createdAt(),c.updatedAt()); }
    private static LibraryViews.Detail toDetail(LibraryContent c) { return new LibraryViews.Detail(c.id(),c.type(),c.title(),c.subject(),c.grade(),c.status(),c.payload(),c.thumbnailUrl(),c.createdAt(),c.updatedAt()); }
    ...
    private static Integer parseGrade(Integer grade) { if (grade == null) return null; if (grade < 10 || grade > 12) throw new IllegalArgumentException("Grade must be 10, 11, or 12."); return grade; }
}
