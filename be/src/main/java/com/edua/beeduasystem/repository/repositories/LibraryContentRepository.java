...
public interface LibraryContentRepository {
    ...
    SummarySearchResult searchSummariesByStatusAndSubject(LibraryContentStatus status, Subject subject, int page, int size);
    ...
    /** Metadata for list views. Intentionally excludes the potentially large JSON payload. */
    record SummarySearchResult(List<LibraryContentSummary> items, long total) { }
    record LibraryContentSummary(UUID id, LibraryContentType type, String title, Subject subject, Integer grade,
                                 String textbookCode, String chapterCode, LibraryContentStatus status,
                                 String thumbnailUrl, Instant createdAt, Instant updatedAt, Instant submittedAt,
