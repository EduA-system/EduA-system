-- Account Management: thêm cột audit cho luồng cấp quyền phân cấp (Principal → Moderator → Teacher).
-- granted_by = UUID của người cấp quyền (REFERENCES app_users), NULL nếu tự seed (principal đầu tiên).
