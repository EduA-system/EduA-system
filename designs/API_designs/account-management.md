...
> Quản lý tài khoản phân cấp: Admin cấp/thu hồi Moderator, Moderator cấp/thu hồi Teacher.
...
- **Phân cấp cứng**: Admin → Moderator → Teacher (3 bậc). Moderator **chỉ quản lý Teacher cùng subject**.
- **Xoá InterimRoleSeedRunner**: không còn seed tài khoản tạm từ env var nữa. Admin sẽ thêm moderator đầu tiên qua API sau khi đăng nhập bằng admin seed.
...
|---|--------|------|------|----|-------|
| 1 | GET | `/api/admin/moderators` | ADMINISTRATOR | UC-60 | Danh sách moderator (phân trang) |
| 2 | POST | `/api/admin/moderators` | ADMINISTRATOR | UC-61 | Thêm moderator = email + subject |
| 3 | DELETE | `/api/admin/moderators/{id}` | ADMINISTRATOR | UC-62 | Thu hồi moderator |
...
### 1. `GET /api/admin/moderators` — Danh sách Moderator
...
### 2. `POST /api/admin/moderators` — Thêm Moderator
...
### 3. `DELETE /api/admin/moderators/{id}` — Thu hồi Moderator
...
- **RBAC**: `@PreAuthorize("hasRole('ADMINISTRATOR')")` cho #1-3; `@PreAuthorize("hasRole('MODERATOR')")` cho #4-6.
...
6. Controller: `AdminController`, `ModeratorController`
7. Xoá `InterimRoleSeedRunner`
8. Sửa `AdminSeedRunner`: thêm `granted_by = null, granted_at = now` (admin đầu tiên tự cấp)
